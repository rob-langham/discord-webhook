...
function typedStepWrapper<Context = obj, Params = obj>(step: IDefineStep) {
  return (
    pattern: string,
    fn: (this: World<Params, Context>, ...args: Params[]) => void | "pending" | Promise<any>
  ) => step(pattern, fn);
}

export function typedSteps<Context = obj, Params = obj>() {
  return {
    given: typedStepWrapper<Context, Params>(Given),
    when: typedStepWrapper<Context, Params>(When),
    then: typedStepWrapper<Context, Params>(Then),
    before: Before, // TODO
